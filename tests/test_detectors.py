...
def test_classifies_known_engine_frameworks():
    path = Path(
        "/Applications/Foo.app/Contents/Frameworks/Electron Framework.framework"
    )
...
def test_classifies_runtime_resources():
    evidence = classify_path(
        Path(
            "/Applications/Foo.app/Contents/Frameworks/Electron Framework.framework/Resources/icudtl.dat"
        ),
        is_executable=False,
    )
...
def test_classifies_locale_pak_as_resource():
    evidence = classify_path(
        Path(
            "/Applications/Foo.app/Contents/Frameworks/Electron Framework.framework/Resources/locales/en-US.pak"
        ),
        is_executable=False,
    )
...
def test_classifies_helper_apps_and_crashpad_handlers():
    helper = classify_path(
        Path("/Applications/Foo.app/Contents/Frameworks/Foo Helper.app"),
        is_executable=False,
    )
    crashpad = classify_path(
        Path("/Applications/Foo.app/Contents/Frameworks/chrome_crashpad_handler"),
        is_executable=True,
    )
...
def test_classifies_contents_macos_executable_entrypoint():
    evidence = classify_path(
        Path("/Applications/Foo.app/Contents/MacOS/Foo"), is_executable=True
    )
...
def test_ignores_unrelated_files():
    assert (
        classify_path(
            Path("/tmp/Foo.app/Contents/Resources/readme.txt"), is_executable=False
        )
        is None
    )
